...
function fixImprovementResult(value: string): ENUMS.IMPROVEMENT_TYPE {
...
interface ProductParams extends IObject.ObjectParams {
    productID?: string;
    ...
    // RnD success model
    minEffectiveDevBudget: number;
    minRnDProjectMaturity: number;
    ...
    payments: {
        ...
        prodPlanning: ENUMS.PaymentArray;
    };
...
interface DevResult {
    devBudget: number;
    improvementResult: ENUMS.IMPROVEMENT_TYPE;
}

export default class Product extends IObject.IObject {
    ...
    private localStocks: Warehouse.Warehouse[];

    insurance: Insurance = null;

    economy: Economy;

    private devStats = new Collections.Stack<DevResult>();
    private notImplementedRnd = new Collections.Queue<number>();
    ...
    restoreLastState(lastResults: Scenario.productRes[], lastDecs: Scenario.product[]) {
        let i = 0;
        let len = Math.max(lastResults.length, lastDecs.length);

        this.lastQualityScores = [];

        let res: Scenario.productRes;
        let dec: Scenario.product;
    ...
    init(semiProducts: SemiProduct[], economy: Economy, lastResults: Scenario.productRes[], lastDecs: Scenario.product[]) {
        super.init();
    ...
    registerLocalStock(stock: Warehouse.Warehouse) {
    ...
    isImprovementsImplemented: boolean;

    lastDevBudgets: number[];
    lastImprovementResults: ENUMS.IMPROVEMENT_TYPE[];
    lastDevProgress: number;

    lastQualityScore: number;
    lastQualityScores: number[];

    lastRnDQuality: number;
    lastAssemblyQuality: number;
    lastMaterialsQuality: number;

    lastRnDImplementDecision: boolean;

    // result
    wantedNb: number;
    manufacturedNb: number;

    // this is just manufactured - rejected - lost
    get producedNb(): number {
    ...
    // actions
    produce(quantity: number, ...semiProductsDecisions: number[]): number {
    ...
    inspect(quantity: number): number {
        if (! Utils.isNumericValid(quantity)) {
    ...
    get improvementResult(): string {
        let result: ENUMS.IMPROVEMENT_TYPE;
        ...
        } else if (RnDProjectProgress - this.lastDevProgress > MINOR_STEP && this.developmentBudget > 0) { // il a depensé so he deserves
    ...
    get majorNotYetImplementedNb(): number {
        return this.notImplementedRnd.size();
    }

    get isMajorImplementedLastPeriod(): boolean {
     ...
     * - Aging of product
    */
    get RnDQuality(): number {
        let quality: number = 0;
        let scoreBeforeAging: number;
        let agingImpact;
