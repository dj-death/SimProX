...
export function IRR(values: number[], guess: number): number {
    ...
    let irrResult = function (values: number[], dates: number[], rate: number): number {
        let r = rate + 1;
        let result = values[0];
...
export function growthRate(income: number, period: number, benchValue: number, guess: number = 0.001): number {
