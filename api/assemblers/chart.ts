...
export function shelfSpaceShare(allResults) {
    return generateChartData(allResults, function (company) {
...
export function totalInvestment(allResults) {
    return generateChartData(allResults, function (company) {
        return company.c_TotalSpending;
    })
}

export function netProfitByCompanies(allResults) {
    return generateChartData(allResults, function (company) {
        return company.c_NetProfit;
    })
}

export function returnOnInvestment(allResults) {
    return generateChartData(allResults, function (company) {
        return company.c_ReturnOnInvestment;
    })
}

export function investmentsVersusBudget(allResults, simulationSpan) {
...
export function marketSalesVolume(allResults) {
    return generateChartData(allResults, function (company) {
        return company.c_MarketSalesVolume[consts.ConsumerSegmentsMaxTotal - 1];
    })
}

export function totalInventoryAtFactory(allResults) {
    return generateChartData(allResults, function (company) {
        return company.c_FactoryStocks[consts.StocksMaxTotal].s_Volume;
    })
}

export function totalInventoryAtTrade(allResults) {
    return generateChartData(allResults, function (company) {
...
export function segmentsLeadersByValue(allResults, segment) {
    let result = [];
...
//Market evolution
export function growthRateInVolume(allResults) {
    return extractMarketEvolutionChartData(allResults, function (market) {
        return market.m_ChangeInVolume;
    });
};

export function growthRateInValue(allResults) {
    return extractMarketEvolutionChartData(allResults, function (market) {
        return market.m_ChangeInValue;
    });
};

export function netMarketPrice(allResults) {
    return extractMarketEvolutionChartData(allResults, function (market) {
        return market.m_ChangeInNetMarketPrice;
    });
};

export function segmentValueShareTotalMarket(allResults) {
...
export function perceptionMap(allResults, exogenous) {
    ...
    allResults.forEach(function (onePeriodResult) {
        ...
        for (let i = 0; i < onePeriodResult.p_Companies.length; i++) {
            ...
            //SKU data
            /*for (let k = 0; k < onePeriodResult.p_SKUs.length; k++) {
                let SKU = onePeriodResult.p_SKUs[k];
                let brand = utility.findBrand(onePeriodResult, SKU.u_ParentBrandID);
...
export function inventoryReport(allResults) {
