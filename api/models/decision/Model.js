...
const decisionSchema = require('./Schema');
...
/*
export function addDecisions (options) {
    var deferred = q.defer();
...
*/
function addDecisions(options, raw) {
    var deferred = q.defer();
    var startFromPlayerID = options.startFromPlayerID, endWithPlayerID = options.endWithPlayerID;
    var startFromPeriod = options.startFromPeriod, endWithPeriod = options.endWithPeriod;
    var docs = [];
    var rawRecordsNb;
    ...
    for (var ID = startFromPlayerID; ID <= endWithPlayerID; ID++) {
        for (var j = startFromPeriod; j <= endWithPeriod; j++) {
            console.log('adding decision of period:' + j + ' , ' + 'playerID:' + ID);
            var dec, res;
            if (isRepeatClone) {
                dec = raw.decision;
                res = raw.results;
    ...
    bulk.execute(function (err, result) {
        ...
        deferred.resolve({
            msg: 'empty decisions, seminar:' + options.seminar + 'import done',
            options: options
        });
    });
    return deferred.promise;
