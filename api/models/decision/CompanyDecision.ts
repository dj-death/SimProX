...
import tDecisionSchema = require('./CompanyDecSchema');
...
export let query = CompanyDecision;
...
export function remove (seminarId, companyId) {
...
export function findOne (seminarId, period, companyId) {
    if (!mongoose.connection.readyState) {
        throw new Error("mongoose is not connected.");
    }
...
export function findAllInPeriod (seminarId, period) {
    if (!mongoose.connection.readyState) {
        throw new Error("mongoose is not connected.");
    }
 ...
 */
export function insertEmptyCompanyDecision (seminarId, period) {
    if (!mongoose.connection.readyState) {
        throw new Error("mongoose is not connected.");
    }
