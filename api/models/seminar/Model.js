...
let q = require('q');
let userRoles = require('../../app/js/routingConfig').userRoles;
...
const seminarSchema = require('./Schema');
        ...
        if (!doc) {
            ...
            doc.remove(function (err, doc) {
                if (err) {
                    ...
                    require('./playerDecision.js').removePlayersDecisions({
                        seminar: req.body.seminarCode
                    }).then(function () {
                        res.send(200, 'delete');
                    }, function (error) {
                        console.error(error.msg);
                        res.send(400, 'remove decision of seminar failure.');
                    }).done();
                }
            });
        }
            ...
            if (req.params.userType == "P") {
                for (var i = 0; i < doc.players[req.params.playerID - 1].reportPurchaseStatus.length; i++) {
                    if (doc.players[req.params.playerID - 1].reportPurchaseStatus[i].period == req.params.period) {
                        res.send(200, doc.players[req.params.playerID - 1].reportPurchaseStatus[i]);
                        break;
                    }
                }
            }
